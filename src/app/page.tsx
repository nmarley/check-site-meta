...
function Header(props: {
    ...
    return (
        <header
            className="grid grid-rows-[1fr] closed:grid-rows-[0fr] group overflow-hidden transition-[grid-template-rows] duration-700"
...
function Footer(props: ComponentProps<'footer'>) {
    return (
        <footer
            {...props}
            className={cn(
                ' w-full col-span-2 pb-[100vh] pt-10 border-t border-border bg-background shadow-2xl',
                props.className,
            )}
        >
            <div className="container-md lg:container-2xl px-8 lg:px-12 xl:px-24 text-foreground-body flex flex-wrap gap-y-8">
                <div className="flex flex-col grow font-mono">
                    ...
                    <div className="text-xs">{getVersion()}</div>
                    <div className="mt-10 flex flex-wrap gap-6">
                        {[
                            [
                                'npm',
                                'https://www.npmjs.com/package/check-site-meta',
                            ],
                            [
                                'github',
                                'https://github.com/alfonsusac/check-site-meta',
                            ],
                            [
                                'twitter',
                                'https://x.com/alfonsusac/status/1899798175512412648',
                            ],
                            ['discord', 'https://discord.gg/DCNgFtCm'],
                        ].map((e) => (
                            <a
                                key={e[0]}
                                className="button transition underline"
                                href={e[1]}
                                target="_blank"
                            >
                                {e[0]}
                    ...
                    <div className="mt-4">
                        Made by <a href="https://alfon.dev">alfonsusac</a> • ©
                        {new Date().getFullYear()} alfonsusac. All rights
                        reserved.
                    </div>
                </div>
...
function Changelog(props: {
    ...
    return (
        <div
            className="w-full grid grid-rows-[1fr] closed:grid-rows-[0fr] overflow-hidden group transition-[grid-template-rows] duration-700"
            data-closed={props.hidden ? '' : undefined}
        >
            <div
                className="min-h-0 closed:opacity-0 transition-all duration-300 delay-100"
                data-closed={props.hidden ? '' : undefined}
            >
                        ...
                        <Fragment key={version}>
                            ...
                            <ul className="">
                                {changes.map((change, i) => (
                                    <li
                                        key={i}
                                        className="text-foreground-muted-2/80 py-0.5 list-['-___']"
                                    >
                                        {change}
                                    </li>
                                ))}
...
// DEBUG
async function RawHTML(prop: { url: string | string[] | undefined }) {
    const getHTML = async () => {
